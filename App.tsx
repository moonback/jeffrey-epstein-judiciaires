...
import { Zap, Play, RotateCw, Database, Search, Activity, Globe, Terminal, Trash2, Layout, Plus, XCircle, Settings } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const processQueue = async () => {
    ...
    const processItem = async () => {
      ...
      // Add temp result to UI and DB immediately
      setResolutionHistory(prev => {
        const updated = [...prev, tempResult];
        return updated;
      });
  ...
  return (
    <div className="min-h-screen bg-[#0F0F0F] text-[#E3E3E3] p-6 font-sans flex flex-col">
      ...
      <main className="max-w-[1600px] mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-6 items-start flex-1">
          ...
          <div className="flex items-center gap-1 overflow-x-auto pb-0 mb-0 no-scrollbar border-b border-[#444746]">
             {resolutionHistory.map((res) => (
                <div 
                    ...
                    className={`group flex items-center gap-2 px-4 py-2.5 rounded-t-lg border-t border-r border-l border-b-0 cursor-pointer min-w-[160px] max-w-[240px] transition-all relative ${
                        activeTabId === res.id 
